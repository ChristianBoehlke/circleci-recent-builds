...
function Build(props: BuildProps) {
  ...
  return (
    <Box>
      <Color
        green={build.outcome === 'success'}
        red={build.outcome === 'failed'}
      >
        <Text>{build.reponame.padEnd(sizeRepo + 1)}</Text>
        <Text>{String(build.build_num).padEnd(sizeBuildNumber + 1)}</Text>
        <Text>{build.branch.padEnd(sizeBranch + 1)}</Text>
        <Progress
          build={build}
...
function Progress(props: ProgressProps) {
  ...
  const fraction = -moment(build.start_time).diff() / estimatedTime;
